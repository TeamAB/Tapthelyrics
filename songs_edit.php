<?php require_once('header.php'); 
  ...
  if( !empty($_GET['id']) )
  {
    $existSong = $controller->getSongById($_GET['id']);
  }
    ...
    !empty( $_FILES["file"]["name"][1] ) )
  {
    ...
    if ( /*($_FILES["file"]["type"][0] == "audio/mpeg") && */ in_array($extension, $allowedExts))
    {
      ...
      else
      {
          ...
          if(file_exists( $existSong->songFile ) )
          {
            unlink( $existSong->songFile );
          }

          if(file_exists( $upload_dir . $_FILES["file"]["name"][0]) )
          {
            unlink($upload_dir . $_FILES["file"]["name"][0]);
          }
    ...
    if ( ($_FILES["file"]["type"][1] == "text/plain") && in_array($extension, $allowedExts))
    {
      ...
      else
      {
        ...
        if(file_exists( $existSong->lyricsFile ) )
        {
          unlink( $existSong->lyricsFile );
        }

        if(file_exists( $upload_dir . $_FILES["file"]["name"][1]) )
        {
          unlink($upload_dir . $_FILES["file"]["name"][1]);
        }
    ...
    {
      echo "Invalid Text file =  ".$_FILES["file"]["type"][1];
    }
                  ...
                  <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <table class="table table-bordered" id="modalUser"> 
                      <tbody>
                          <tr>
                            <td>Title:</td>
                            <td><input type="text" name="title" value="<?php echo $existSong->title; ?>" required id="title"/></td>
                          </tr>
                         <tr>
                          <td>Artist:</td>
                          <td><input type="text" name="artist" value="<?php echo $existSong->artist; ?>" required id="artist"/></td>
                        </tr>
                        <tr>
                          <td>Song File:</td>
                        ...
                        <tr>
                          <td>Difficulty:</td>
                          <td>
                              <select class="combobox" name="difficulty" value="<?php echo $existSong->difficulty; ?>" style="height:30px;width:206px;">
                    ...
                    <button class="btn btn-danger" id="btnSave">Edit Song</button>
